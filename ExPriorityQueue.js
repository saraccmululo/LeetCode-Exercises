class PriorityQueue {
  constructor() {
    this.values = [];
  }
  enqueue(val, priority) {//another name for insert()
    let newNode=new Node(val, priority);
    this.values.push(newNode);
    this.bubbleUp();
  }
  bubbleUp() {
    let idx = this.values.length - 1;
    const element = this.values[idx];
    while (idx > 0) {
      //stops when index reaches 0 because there is no more elements before index 0 to be compared.
      let parentIdx = Math.floor((idx - 1) / 2);
      let parent = this.values[parentIdx];
      if (element.priority >= parent.priority) break; //found the correct spot, end the loop.
      //if not, swap element-parent value and index:
      this.values[parentIdx] = element;
      this.values[idx] = parent;
      idx = parentIdx; //move index to the next comparison
    }
  }
  dequeue() {//another name for remove()
    const min = this.values[0];
    const end = this.values.pop();
    if (this.values.length > 0) {
      this.values[0] = end;
      this.bubbleDown();
    }
    return min;
  }

  bubbleDown() {
    let idx = 0;
    const length = this.values.length;
    const element = this.values[0];
    while (true) {
      let leftChildIdx = 2 * idx + 1;
      let rightChildIdx = 2 * idx + 2;
      let leftChild, rightChild;
      let swap = null;

      if (leftChildIdx < length) {
        //keep child index inside array boundaries(only do the operation if child index is less than array size)
        leftChild = this.values[leftChildIdx];
        if (leftChild.priority < element.priority) {
          //if child lower than parent, swap index
          swap = leftChildIdx;
        }
      }
      if (rightChildIdx < length) {
        rightChild = this.values[rightChildIdx];
        if (
          (swap === null && rightChild.priority < element.priority) ||
          (swap !== null && rightChild.priority < leftChild.priority)
        ) {
          swap = rightChildIdx;
        }
      }
      if (swap === null) break;
      this.values[idx] = this.values[swap];
      this.values[swap] = element;
      idx = swap; //update the index to keep walking through the array
    }
  }
  print() {
  return this.values.map(n => `${n.val} (${n.priority})`);
 }
}

class Node {
  constructor(val, priority){
    this.val=val;
    this.priority=priority;
  }
}

let priorityQueue = new PriorityQueue();
priorityQueue.enqueue("common cold", 5);
priorityQueue.enqueue("gunshot wound", 1)
priorityQueue.enqueue("high fever",3)
priorityQueue.enqueue("broken arm", 2)
priorityQueue.enqueue("cut in foot",3)
priorityQueue.dequeue();

console.log(priorityQueue.print());
